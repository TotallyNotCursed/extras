...
public class ModOreFeatures {
    private static final ResourceKey<PlacedFeature> ORE_COLDVEIN_SMALL_KEY =
            ResourceKey.create(Registries.PLACED_FEATURE, ModUtils.getResourceLocation("ore_coldvein_small"));
    private static final ResourceKey<PlacedFeature> ORE_COLDVEIN_LARGE_KEY =
            ResourceKey.create(Registries.PLACED_FEATURE, ModUtils.getResourceLocation("ore_coldvein_large"));
    private static final Logger LOGGER = LoggerFactory.getLogger("TNC Extras Ore Feature Registration");

    public static void register() {
        LOGGER.info("Registering ore features");

        BiomeModifications.addProperties((ctx) -> ctx.hasTag(BiomeTags.IS_OVERWORLD), (ctx, mutable) -> {
