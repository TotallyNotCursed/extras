    ...
    public void tick() {
        ...
        if (nearest != null && ModUtils.isNightOrDark(this)) {
            ...
            if (isSeen) {
                ...
                nearest.addEffect(new MobEffectInstance(MobEffects.MOVEMENT_SLOWDOWN, SLOW_DURATION, SLOW_AMPLIFIER));
                nearest.addEffect(new MobEffectInstance(MobEffects.BLINDNESS, BLINDNESS_DURATION, BLINDNESS_AMPLIFIER));
                nearest.addEffect(new MobEffectInstance(MobEffects.WITHER, WITHER_DURATION, WITHER_AMPLIFIER));
                this.level()
                        .playSound(null, blockPosition(), SoundEvents.ENDERMAN_STARE, SoundSource.HOSTILE, 1.0f, 0.7f);
                ...
                if (this.tickCount % TELEPORT_CHECK_INTERVAL == 0
                    ...
                    if (this.random.nextFloat() < TELEPORT_CHANCE) {
                        Vec3 direction =
                                nearest.position().subtract(this.position()).normalize();
        ...
        if (visibleTicks > 0) {
            ...
            if (nearest != null && ModUtils.isNightOrDark(this) && !isSeen) {
                if (this.tickCount % PARTICLE_SPAWN_INTERVAL == 0) {
                    this.level().broadcastEntityEvent(this, (byte) PARTICLE_EVENT_ID); // Trigger particle event
                }
            }
        }
